...
alias du='du -h'
alias less='less -M'
alias kall='sudo killall'
alias radio='pyradio -s ~/Music/stations.csv'
alias nano='nano --smooth --tabstospaces --nowrap --autoindent --tabsize=4'
alias upd='pip install --upgrade --user youtube-dl pyradio pip && sudo apt -y update && sudo apt -y upgrade && sudo apt-get -y autoremove && sudo apt-get -y autoclean'
alias r='make -s && ./a.out'
alias yt='youtube-dl -f 22 -i -o "%(autonumber)s. %(title)s.%(ext)s" --no-playlist'
alias ytp='youtube-dl -f 22 -i -o "%(playlist_index)s. %(title)s.%(ext)s"'
